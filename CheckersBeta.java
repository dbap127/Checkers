...
class Checkers
{
  ...
  public static int origSL;
  public static int origSN; 
  ...
  public static void sequenceCheck(String i) // method that detects if multi movements or single movements are valid
  {
    ...
    try
    {
      if (i.matches("^[a-hA-H0-8]+$") && (i.length() % 2) == 0)  // detects if input only contains a to h and 0 to 8
      {
        ...
        origSL = rowLegend.indexOf(i.charAt(0));
        origSN = columnLegend.indexOf(i.charAt(1));
        ...
        while (i.length() > 3) // for multi hops, keep checking inputs
        {
          ...
          else if (spaces[origSL][origSN] == '0')
          {
            if(endLetter == (startLetter + 1) && endNumber == (startNumber + 1) && spaces[endLetter][endNumber] == ' ') // checking if its a single move
            {
              moveSeq = moveSeq.concat("1");
          ...
          else if (spaces[origSL][origSN] == 'k' || spaces[origSL][origSN] == 'K')
          {
        ...
        // System.out.println(moveSeq); for debugging
